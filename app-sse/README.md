...
|---------|------------------------|---------------|-----------------|
| 아키텍처    | 인메모리 데이터 저장소 + Pub/Sub | 큐 기반 메시지 브로커  | 분산 이벤트 스트리밍 플랫폼 |
| 지원 프로토콜 | Pub/Sub                | AMQP          | Pub/Sub         |
| 메시지 큐   | X                      | O             | O               |
| 메시지 순서  | 보장하지 않음                | 큐 내에서 FIFO    | 파티션 내 순서 보장     |
| 처치량     | 초당 100만+ 메시지           | 초당 38MB       | 초당 605MB        |
| 지연시간    | 매우 낮음                  | ~1ms (낮은 부하)  | ~5ms            |
| 메시지 영속성 | 제한적 (인메모리 특성)          | 디스크 저장 지원     | 강력한 영속성 보장      |
| 확장성     | 메모리 한계, 클러스터링 가능       | 수평적 확장 가능     | 매우 높은 확장성       |
| 사용 사례   | 실시간 메시징, 캐싱            | 작업 큐, 실시간 메시징 | 로그 수집, 스트림 처리   |
| 복잡도     | 단순함                    | 중간            | 높음              |
| 내구성     | 낮음                     | 중간            | 높음              |

**최적 사용 시나리오**

* `Redis`: 짧은 수명의 메시지, 실시간 처리가 필요한 경우
